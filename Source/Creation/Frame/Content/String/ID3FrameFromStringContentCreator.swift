...
class ID3FrameFromStringContentCreator: FrameFromStringContentCreator {
    ...
    func createFrame(frameIdentifier: [UInt8], version: ID3Version, content: String) -> [UInt8] {
        ...
        let contentAsBytes = stringToBytesAdapter.adapt(
            string: content, for: version)
        frame.append(contentsOf: frameContentSizeCalculator.calculateSizeOf(
            content: contentAsBytes, version: version))
        frame.append(contentsOf: frameFlagsCreator.createFor(
            version: version))
