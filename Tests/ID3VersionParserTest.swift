...
class ID3VersionParserTest: XCTestCase {
    ...
    func testExtractVersion2() {
        let mp3WithV2Tag = NSData(contentsOfFile: PathLoader().pathFor(name: "example", fileType: "mp3"))
        XCTAssertEqual(2, id3VersionParser.parse(mp3: mp3WithV2Tag!))
    }
    
    func testExtractVersion3() {
        let mp3WithV2Tag = NSData(contentsOfFile: PathLoader().pathFor(name: "example-v23", fileType: "mp3"))
        XCTAssertEqual(3, id3VersionParser.parse(mp3: mp3WithV2Tag!))
    }
    
    func testDefaultVersion() {
        let mp3WithV2Tag = NSData(contentsOfFile: PathLoader().pathFor(name: "example-to-be-modified", fileType: "mp3"))
        XCTAssertEqual(3, id3VersionParser.parse(mp3: mp3WithV2Tag!))
    }
}
